...
function generate_array(){
  ...
  for(let i=0;i<arr_size;i++){
    div_sizes[i] = Math.floor(Math.random() * 0.8 * (arr_size_input.max - arr_size_input.min)) + 10;
    divs[i] = document.createElement("div");
    arr_cont.appendChild(divs[i]);
    margin_size = 0.1;
    divs[i].style = " margin:0% " + margin_size + "%; background-color:#87CEEB; width:" + (100 / arr_size - (2 * margin_size)) + "%; height:" + (div_sizes[i]) + "%;";
  }
}
